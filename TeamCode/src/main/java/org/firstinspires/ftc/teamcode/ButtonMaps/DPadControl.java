...
public class DPadControl {
    public static MotorPowers dpadStrafe(OpMode opMode, double multiplier) {
        ...
        if (opMode.gamepad1.dpad_down) {
            ...
            if (opMode.gamepad1.dpad_left) mp = HolonomicDrive.robotOrientedDrive(-1, -1, 0, multiplier);
            //When dpad down-right is pressed, move diagonally down-right
            else if (opMode.gamepad1.dpad_right) mp = HolonomicDrive.robotOrientedDrive(1, -1, 0, multiplier);
            //When dpad down is pressed, move straight down
            else mp = HolonomicDrive.robotOrientedDrive(0, -1, 0, multiplier);
        ...
        else if (opMode.gamepad1.dpad_right) mp = HolonomicDrive.robotOrientedDrive(1, 0, 0, multiplier);
        //When dpad left is pressed, move straight left
        else if (opMode.gamepad1.dpad_left) mp = HolonomicDrive.robotOrientedDrive(-1, 0, 0, multiplier);
