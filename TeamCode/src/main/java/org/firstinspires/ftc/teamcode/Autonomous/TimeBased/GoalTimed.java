...
public class GoalTimed extends LinearOpMode {
    ...
    enum Direction {FORWARD, BACKWARD, LEFT, RIGHT, POSITIVE, NEGATIVE, COMMON}
    Direction dir;
...
//        telemetry.update();
//        sleep(500);
        driveAllMotorsTo(Direction.FORWARD, 800, System.currentTimeMillis(), .8, robot);
...
//        sleep(500);
        robot.setMotorPower(0,0,0,0);
        rotateTo(Direction.POSITIVE, 1055, System.currentTimeMillis(), .5, robot);
...
//        aimLimelight(robot);
        ...
        while (System.currentTimeMillis() < timeSet + 17000) {
            double targetVel = -FirstAgeArm.velocityShot(120);
            double shootVel = robot.ShootMotor.getVelocity();
            ShootingFunctions.setVelocity(targetVel, shootVel, robot.ShootMotor, .95);
...
//        robot.Servo3.setPower(0);
        ...
        strafeMotorsTo(dir, 400, System.currentTimeMillis(), .8, robot);
        robot.setMotorPower(0,0,0,0);
        rotateTo(Direction.NEGATIVE, 300, System.currentTimeMillis(), .5, robot);
